...
const runChat = createServerFn({ method: "POST" }).handler(async ({ data }) => {
			...
			<div className="max-w-6xl mx-auto px-6 py-12 space-y-10">
				<header className="space-y-3 text-center">
					...
					<p className="text-lg text-gray-300 max-w-3xl mx-auto">
						Authenticate with Auth0, stream responses from OpenAI via the AI
						SDK, and showcase outputs using the ai-elements component
						primitives.
					</p>
				</header>
				...
				<Card className="bg-slate-900/70 border-slate-800">
					<CardHeader className="flex flex-col gap-2">
						...
						<p className="text-sm text-gray-300">
							Powered by <code className="text-cyan-300">@ai-sdk/openai</code>{" "}
							with streaming thinking traces and provider source metadata.
						</p>
					</CardHeader>
						...
						<div className="grid gap-4 md:grid-cols-[1.2fr_0.8fr]">
							...
							<div className="space-y-3 rounded-lg border border-slate-800 bg-slate-950/60 p-4 text-sm text-gray-300">
								...
								<ul className="list-disc space-y-2 pl-5">
									<li>
										Auth0 protects AI interactions while keeping the router
										stateful.
									</li>
									<li>
										AI SDK streams OpenAI responses with reasoning and sources
										enabled.
									</li>
									<li>
										ai-elements primitives render conversations with TanStack
										styling.
									</li>
								</ul>
								<p className="text-xs text-gray-400">
									Configure <code>OPENAI_API_KEY</code> to stream real outputs
